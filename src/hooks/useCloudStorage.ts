 ...
 * @group Hooks
 */
const useCloudStorage = (): { getItem: GetItemFunction; setItem: SetItemFunction; getItems: GetItemsFunction; removeItem: RemoveItemFunction; getKeys: GetKeysFunction } => {
  const cloudStorage = useWebApp()?.CloudStorage;

  const getItem: GetItemFunction = useCallback( key =>
      new Promise((resolve, reject) => {
        cloudStorage?.getItem(key, (error:any, value:any) => {
          if (!error) {
            resolve(value);
          } else {
            reject(error);
  ...
  const getItems: GetItemsFunction = useCallback( key =>
      new Promise((resolve, reject) => {
        cloudStorage?.getItems(key, (error:any, value:any) => {
          if (!error && value) {
            resolve(value);
          } else {
            reject(error);
  ...
  const getKeys: GetKeysFunction = useCallback( () =>
      new Promise((resolve, reject) => {
        cloudStorage?.getKeys((error:any, state:any) => {
          if (!error && state) {
            resolve(state);
          } else {
            reject(error);
  ...
  return useMemo( () => ({ getItem, setItem, getItems, removeItem, removeItems, getKeys }),
